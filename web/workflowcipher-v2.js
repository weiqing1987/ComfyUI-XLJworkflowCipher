...
async function requestJson(url, options = {}) {
  ...
  if (storedToken && !Object.keys(headers).some((name) => name.toLowerCase() === "authorization")) {
...
const TOKEN_STORAGE_KEY = "xljworkflowcipher_api_token";
const API_USERNAME_STORAGE_KEY = "xljworkflowcipher_api_username";
...
function getStoredApiUsername() {
  try {
    return localStorage.getItem(API_USERNAME_STORAGE_KEY) || "";
  } catch (_e) {
    return "";
  }
}
...
function setStoredApiUsername(username) {
  try {
    if (username) {
...
async function getRemoteAccountSummary(forceRefresh = false) {
  ...
  try {
    ...
    const payload = await requestBackendJson("/xljworkflowcipher/api/me", {}, config);
...
async function remoteLogout() {
  try {
    ...
    console.warn(`${BRAND_NAME} logout failed:`, error);
  } finally {
    clearStoredAuth();
  }
  return { loggedIn: false, username: "" };
...
async function requestBackendJson(path, options = {}, config = frontendConfigCache) {
  const apiBase = normalizedApiBase(config);
  const remoteUrl = apiBase ? `${apiBase}${path}` : "";
  const preferRemote = Boolean(remoteUrl) && (hasCrossOriginApiBase(config) || Boolean(getStoredApiToken()));

  if (preferRemote) {
    try {
      return await requestRemoteJson(remoteUrl, options);
    } catch (remoteError) {
      try {
...
async function getFrontendConfig(forceRefresh = false) {
  if (!frontendConfigPromise || forceRefresh) {
    frontendConfigPromise = (async () => {
      try {
        const payload = await requestJson(FRONTEND_CONFIG_PATH);
        frontendConfigCache = {
          ...frontendConfigCache,
          ...(payload || {}),
  ...
  return new Promise((resolve) => {
    ...
    panel.innerHTML = `
      <div>
        ...
        <p style="margin:10px 0 0;color:#9fb0d0;line-height:1.6;">
          使用创作者后台账号登录，获取 API Token 用于跨域认证。
        </p>
      </div>
    ...
    panel.addEventListener("submit", async (event) => {
      ...
      try {
        ...
        if (data?.api_token) {
          rememberAuthPayload(data);
          window.alert("登录成功！API Token 已保存，可以正常加密了。");
          cleanup({
            loggedIn: true,
            username: data?.user?.username || username,
          });
        } else {
          window.alert("登录成功但未获取到 API Token。");
          cleanup(false);
        }
      } catch (error) {
  ...
  return new Promise((resolve) => {
    ...
    panel.innerHTML = `
      <div>
        ...
        <div data-auth-row style="margin-top:14px;display:flex;align-items:center;justify-content:space-between;gap:12px;flex-wrap:wrap;padding:12px 14px;border-radius:14px;background:rgba(255,255,255,0.04);">
          <div style="display:grid;gap:4px;">
            <span style="color:#71d9cb;font-size:12px;letter-spacing:0.08em;text-transform:uppercase;">后台账号</span>
            <span data-auth-text style="color:#eef3ff;font-size:14px;">检查登录状态...</span>
          </div>
          <div style="display:flex;gap:8px;flex-wrap:wrap;">
            <button type="button" data-action="remote-login" style="padding:11px 14px;border-radius:14px;border:1px solid rgba(255,255,255,0.08);background:#2d5a4e;color:#eef3ff;">登录后台</button>
            <button type="button" data-action="remote-logout" style="display:none;padding:11px 14px;border-radius:14px;border:1px solid rgba(255,255,255,0.08);background:transparent;color:#eef3ff;">退出登录</button>
          </div>
        </div>
      </div>
      <label style="display:grid;gap:8px;">
      ...
      <div style="display:grid;gap:10px;">
        ...
        <button type="button" data-action="portal-link" style="justify-self:start;padding:0;border:0;background:transparent;color:#71d9cb;font-size:13px;text-decoration:underline;cursor:pointer;">打开后台网站</button>
      </div>
    `;
    ...
    const authText = panel.querySelector("[data-auth-text]");
    const remoteLoginButton = panel.querySelector('[data-action="remote-login"]');
    const remoteLogoutButton = panel.querySelector('[data-action="remote-logout"]');
    ...
    function renderAuthState(account = null) {
      const cachedAccount = getCachedAccountSummary();
      const username = String(account?.username || cachedAccount.username || "").trim();
      const loggedIn = Boolean(account?.loggedIn || cachedAccount.loggedIn);
      if (username) {
        authText.textContent = `已登录账号：${username}`;
        remoteLoginButton.style.display = "none";
        remoteLogoutButton.style.display = "inline-flex";
        return;
      }
      if (loggedIn) {
        authText.textContent = "已登录，正在同步账号信息...";
        remoteLoginButton.style.display = "none";
        remoteLogoutButton.style.display = "inline-flex";
        return;
      }
      authText.textContent = "未登录，启用密钥系统前请先登录后台账号。";
      remoteLoginButton.style.display = "inline-flex";
      remoteLogoutButton.style.display = "none";
    }
    ...
    remoteLoginButton.addEventListener("click", async (event) => {
      event.preventDefault();
      event.stopPropagation();
      const account = await remoteLogin();
      renderAuthState(account);
    });
    remoteLogoutButton.addEventListener("click", async (event) => {
      event.preventDefault();
      event.stopPropagation();
      const account = await remoteLogout();
      renderAuthState(account);
    });
    overlay.addEventListener("click", (event) => {
      if (event.target === overlay) {
    ...
    void getRemoteAccountSummary().then((account) => renderAuthState(account));
...
if (!globalThis[MODULE_GUARD_KEY]) {
  globalThis[MODULE_GUARD_KEY] = true;
  ...
  app.registerExtension({
    ...
    setup() {
      console.info(`${BRAND_NAME} build ${BUILD_ID} loaded`);
